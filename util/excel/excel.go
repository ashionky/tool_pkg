 ...
 * @return err      {error}                    异常
 */
func ExportExcel(name, path string, header []string, headerKV map[string]string, data []map[string]interface{}) (fileName string, err error) {
	f := excelize.NewFile()
	// Create a new sheet
	...
	fileNamePath := path + fileName //保存文件的位置
	err = f.SaveAs(fileNamePath)
	return
}

/**
 * 读取Excel表格 返回list，可改用结构体list,如需要，可封装后提交一个版本
 * @param  filePath     {string}    表路径
 * @param  header   {[]string}     表头key
 * @return       {[]map[string]interface{}} 数据集合
 * @return       {error}                     异常
 */
func ReadExcel(filePath string, head []string) ([]map[string]interface{}, error) {
