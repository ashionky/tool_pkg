...
func (db *DB) SetMaxOpenConnections(max []int) {
	if len(db.sqlxdb) != len(max) {
		panic("unmatched count of db and set maxOpenConnections")
	}
	for i := range db.sqlxdb {
		db.sqlxdb[i].SetMaxOpenConns(max[i])
	}
}
...
func (db *DB) SetConnMaxLifetime(d []time.Duration) {
	if len(db.sqlxdb) != len(d) {
		panic("unmatched count of db and set maxLifeTime")
	}
	for i := range db.sqlxdb {
		db.sqlxdb[i].SetConnMaxLifetime(d[i])
	}
}
...
func (db *DB) SetMaxIdleConns(maxIdleConns []int) {
	if len(db.sqlxdb) != len(maxIdleConns) {
		panic("unmatched count of db and set maxIdleConns")
	}
	for i, val := range db.sqlxdb {
		val.SetMaxIdleConns(maxIdleConns[i])
	}
}
